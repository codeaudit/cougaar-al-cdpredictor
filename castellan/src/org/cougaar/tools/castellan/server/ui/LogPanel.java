...
public class LogPanel extends JPanel implements AppComponent {
    ...
    public class DatabaseLogPanel extends JPanel implements LogObserver {
        DatabaseLogPanel( PersistentEventLog log ) {
            this.log = log ;
            buildLayout() ;
        }
        ...
        public void buildLayout() {
            ...
            JScrollPane sp ;
            gbl.setConstraints( sp = new JScrollPane( textPane ), gbc );
            add( sp ) ;
            ...
            gbl.setConstraints( updateButton, gbc );
            add( updateButton ) ;
...
//            add( panel ) ;
        ...
        PersistentEventLog log ;

        JTextField nameLabel ;

        public void update() {
            StringBuffer buf = new StringBuffer() ;
            // Scan the log and profile for event times.
            long firstEventTime = log.getFirstEventTime() ;
            long lastEventTime = log.getLastEventTime() ;
            buf.append( "Number of events= " ).append ( log.getNumEvents() ).append( "\n" ) ;
            buf.append( "Start time=" ) ;
            PDU.formatTime( buf, firstEventTime );
            buf.append( '\n' ) ;
            buf.append( "Stop time=" ) ;
            PDU.formatTime( buf, lastEventTime );
            buf.append( '\n' ) ;

            long firstExecutionTime = log.getFirstExecutionTime() ;
            long lastExecutionTime = log.getLastExecutionTime() ;
            buf.append( "Start execution time=" ) ;
            PDU.formatTime( buf, firstExecutionTime ) ;
            buf.append( '\n' ) ;
            buf.append( "End execution time=" ) ;
            PDU.formatTime( buf, lastExecutionTime ) ;
            buf.append( '\n' ) ;
    ...
    public class InMemoryLogPanel extends JPanel implements LogObserver {
        ...
        protected void doSaveAction() {
            ...
            try {
                ...
                Iterator iter =
                    log.getEvents( log.getFirstEventTime(), log.getLastEventTime() ) ;
                while ( iter.hasNext() )
                {
    ...
    protected void makeNewPanel( EventLog log ) {
        ...
        else if ( log instanceof PersistentEventLog ) {
            logComponent = new DatabaseLogPanel( ( PersistentEventLog ) log ) ;
        }
    }
