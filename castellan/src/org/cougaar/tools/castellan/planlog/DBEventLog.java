...
public class DBEventLog implements EventLog
{
	...
	TaskPDUTable myTaskPDUTable;
	AssetPDUTable myAssetPDUTable;
	PlanElementPDUTable myPlanElementPDUTable;
	AllocResPDUTable myAllocResPDUTable;
    ...
    // CONSTRUCTORS
    /**
     * Default constructor. Create a new DBEventLog with a new database with 
     * the database name = input argument, name.
     *
     * @param name Name of database.  If it already exists, a runtime exception is thrown.
     */
    public DBEventLog ( String name ) {
        this( name, null ) ;
    }

    /**
     * Create a new DBEventLog with a new database and tables.
     * The database name = input argument, name.
     * @param name Name of database.
     */
    public DBEventLog ( String name, Properties props ) {
        if ( name == null ) {
    ...
    /**
     * Create a new DBEventLog with an existing database and tables.
     * The database name = input argument, name.
     ...
     */
    public DBEventLog ( String name, Properties props, boolean useExistingDB ) {
        if ( name == null ) {
    ...
    public static ArrayList getValidDatabases( String dbPath, String user, String password ) {
        ...
        try {
            ...
            throw new RuntimeException( "DBEventLog :: Error connecting to database." );
        }
...
//            return results ;
    ...
    private void connect() {
        ...
        if ( dbPath == null )
        {
            throw new RuntimeException( "DBEventLog :: dbPath variable not set. Could not establish connection to database." );
        }
    ...
    private synchronized void createTables( Statement stmt ){        
       ...
       // myEventPDUTable.createTable( stmt );
		...
		// Instantiate myTaskPDUTable

		myTaskPDUTable = new TaskPDUTable(conn);
		myTaskPDUTable.createTable( stmt );

		// Instantiate myPlanElementPDUTable

		myPlanElementPDUTable = new PlanElementPDUTable(conn);
		myPlanElementPDUTable.createTable( stmt );

		// Instantiate myAssetPDUTable
  
        myAssetPDUTable = new AssetPDUTable(conn);
		myAssetPDUTable.createTable( stmt );
		 
		 //Instantiate myAllocResPSUTable 

        myAllocResPDUTable = new AllocResPDUTable(conn);
		myAllocResPDUTable.createTable(stmt);
    }
    ...
    */
    ...
    /**
        public static void main( String[] args ) {
            ...
            try {
                System.out.println("_______________TESTING DBEventLog _______________");
                DBEventLog log = new DBEventLog ( "bbpelog", myDbProps ) ;
