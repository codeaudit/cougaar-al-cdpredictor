...
public class PDU implements java.io.Serializable {
    ...
    public static String formatElapsedTime( StringBuffer buf, long time ) {
        long ms = time - ( time / 1000 ) * 1000 ;
        time = time / 1000 ;
        long temp ;
        ...
        time = time - temp * 60 ;
        buf.append( "m " ).append( time ) ;
        buf.append( "." ).append( ms ).append( "s " ) ;
        return buf.toString() ;
    }

    public static String formatElapsedTime( long time ) {
        StringBuffer buf = new StringBuffer() ;
        return formatElapsedTime( buf, time );
    }
