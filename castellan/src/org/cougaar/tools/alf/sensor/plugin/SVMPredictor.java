	...
	public void forecast(long today) {
		...
		for (Iterator iter = customers.iterator();iter.hasNext() ; )	{
			...
			for (Iterator iterItem = historyOfItems.iterator();iterItem.hasNext() ; )
			{
				...
				long nextEndTime = demandHistoryForAnItem.nextEstimatedEndtime();

				if (nextEndTime > today + leadTime)	{
					continue;
				}

				// assuming comm loss day is yesterday.
				int timeLag = 3;
				...
				for (int k=0;k<timeLag;k++)	{
					myLoggingService.shout("pastRecord["+k+"]="+pastRecord[k]);
				}
				//
